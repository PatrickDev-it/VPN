...
│       ├── cd.yml                   ← publish to GHCR → deploy to VPS via SSH
...
│
├── app/
│   ├── gatekeeper.py                ← CORE LOGIC (346 lines, mitmproxy addon)
│   ├── policy.yaml                  ← whitelist / blacklist rules (Python regex)
│   ├── users.yaml                   ← user credentials (git-ignored, create manually)
...
│
├── unbound/
│   ├── unbound.conf                 ← DNSSEC, DoT, performance tuning
│   └── blacklist.conf               ← auto-generated DNS sinkhole (StevenBlack/hosts)
│
├── privoxy/config                   ← generated by bootstrap.sh from .env
├── tor/torrc                        ← Tor: entry guards, geo exit policy, circuit tuning
│
├── init/
│   ├── bootstrap.sh                 ← Docker Compose up + systemd + cron setup
│   ├── setup-vps.sh                 ← full Debian 12 host provisioning (run as root)
│   ├── setup-domain.sh              ← interactive domain + HTTPS wizard
│   ├── generate-certs.sh            ← TLS cert generation (selfsigned/letsencrypt/cloudpanel)
│   └── vps-proxy-stack.service      ← systemd unit template
│
├── cron/
│   └── update_blacklist.sh          ← fetch StevenBlack/hosts → Unbound format
│
├── Dockerfile                       ← mitmproxy image (python:3.12-slim)
├── docker-compose.yml               ← orchestrates 4 services
├── Makefile                         ← operational targets
├── .env.example                     ← environment variable template (52 lines)
├── requirements.txt                 ← mitmproxy==11.0.2, PyYAML==6.0.2
└── requirements-dev.txt             ← pytest==8.4.1, ruff==0.12.10
```
...
app/users.yaml        # user credentials — copy from users.yaml.example
...
3. Add block to `app/users.yaml`
4. `docker compose restart mitmproxy`
5. **Update `session.md`** — mandatory

### Add a policy rule
1. Edit `app/policy.yaml` (Python regex, case-insensitive)
2. `docker compose restart mitmproxy`
3. **Update `session.md`** — mandatory

### Modify Tor exit countries
1. Edit `ExitNodes` / `ExcludeNodes` in `tor/torrc`
...
Push to `main` or create a tag `vX.Y.Z`. The CD workflow runs automatically.
...
| `cd.yml` | push to `main` or tag `v*.*.*` | Publish GHCR → SSH deploy |
